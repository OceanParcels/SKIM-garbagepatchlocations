...
filenames = {'U': "/scratch/Victor/TotalData24h/20*.nc",
             'V': "/scratch/Victor/TotalData24h/20*.nc",
	     'uuss':"/scratch/Victor/StokeData24h/StokesDrift*.nc",
	     'vuss':"/scratch/Victor/StokeData24h/StokesDrift*.nc",
...
lons=np.load('/home/students/4056094/Desktop/Thesis/ParcelsOutput/Global/InputDistribution/LonsGeoTestgrid1x1.npy')
lats=np.load('/home/students/4056094/Desktop/Thesis/ParcelsOutput/Global/InputDistribution/LatsGeoTestgrid1x1.npy')
...
def RungeKutta4FullCurrents(particle,fieldset,time,dt):
    ...
    constant=0.00001*(-1)
    d=particle.depth
    u0=constant*fieldset.borU[time,lon0,lat0,d]+fieldset.uuss[time,lon0,lat0,d]
    v0=constant*fieldset.borV[time,lon0,lat0,d]+fieldset.vuss[time,lon0,lat0,d]

    lon1=lon0+u0*dt/2
    lat1=lat0+v0*dt/2
    u1=constant*fieldset.borU[time+0.5*dt,lon1,lat1,d]+fieldset.uuss[time+0.5*dt,lon1,lat1,d]
    v1=constant*fieldset.borV[time+0.5*dt,lon1,lat1,d]+fieldset.vuss[time+0.5*dt,lon1,lat1,d]

    lon2=lon0+u1*dt/2
    lat2=lat0+v1*dt/2
    u2=constant*fieldset.borU[time+0.5*dt,lon2,lat2,d]+fieldset.uuss[time+0.5*dt,lon2,lat2,d]
    v2=constant*fieldset.borV[time+0.5*dt,lon2,lat2,d]+fieldset.vuss[time+0.5*dt,lon2,lat2,d]
    
    lon3=lon0+u2*dt
    lat3=lat0+v2*dt
    u3=constant*fieldset.borU[time+dt,lon3,lat3,d]+fieldset.uuss[time+dt,lon3,lat3,d]
    v3=constant*fieldset.borV[time+dt,lon3,lat3,d]+fieldset.vuss[time+dt,lon3,lat3,d]
...
#%%
pfile = pset.ParticleFile(name="/scratch/Victor/GlobalStokes",
