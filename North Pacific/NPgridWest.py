...
for the North Pacific Basin, the western part
...
def GenerateP(landfilename, lonmin, lonmax, latmin, latmax, spacing, name):
    ...
    #Remove Bay of Bengal
    #Remove pacific part
    [lons,lats]=[np.array([lo for lo, la in zip(lons,lats) if not (lo<100 and la<20)  ]),np.array([la for lo, la in zip(lons,lats) if not (lo<100 and la<20)])]
    [lons,lats]=[np.array([lo for lo, la in zip(lons,lats) if not (lo<103 and la<6.5)  ]),np.array([la for lo, la in zip(lons,lats) if not (lo<103 and la<6.5)])]
    ...
    np.save('D:\Desktop\Thesis\ParcelsFigData\Data\North Pacific\InputArray/LonsWest' + name,lons)
    np.save('D:\Desktop\Thesis\ParcelsFigData\Data\North Pacific\InputArray/LatsWest' + name,lats)
